...
import CustomerTable from "./TicketTable/TicketTable";
...
import styles from './TicketRevenue.module.scss'
...
function TicketRevenue()
{
    ...
    const revenueByOrder = useSelector((state) => state?.Report?.revenueByOrder);
    ...
    const getRevenueByOrder = (dataSubmit) => {
        dispatch({
            type: ReportAction.GET_REPORT_BY_ORDER,
            params: dataSubmit,
            onSuccess: (data) => {
                //
            },
    ...
    return (
        ...
        >
            ...
            <CustomerTable sum={revenueByOrder?.totalRevenueByOrders} paging={revenueByOrder?.paging} data={typeof revenueByOrder?.listRevenueByOrders != 'undefined' ? revenueByOrder?.listRevenueByOrders : []} tableName='Doanh thu theo chi nhánh' /> 
        </div>    
    )
}
