...
import CustomerTable from "./TableBranch/TableBranch";
...
import styles from './ReportBranch.module.scss'
...
function ReportBranch()
{
    ...
    const revenueByBranch = useSelector((state) => state?.Report?.revenueByBranch);
    const listFilm = useSelector((state) => state?.Film?.film);
    const listFilmSelect = typeof listFilm != 'undefined' ? listFilm?.map(item => (
        {
            key: item?.id,
            label: item?.name
        }
    ...
    const getRevenueByBranch = (dataSubmit) => {
        dispatch({
            type: ReportAction.GET_REPORT_BY_BRANCH,
            params: dataSubmit,
            onSuccess: (data) => {
                //
            },
    ...
    return (
        ...
        >
            ...
            <CustomerTable data ={typeof revenueByBranch != 'undefined' && revenueByBranch != null ? revenueByBranch : []} tableName='Doanh thu theo chi nhánh' /> 
        </div>    
    )
}
